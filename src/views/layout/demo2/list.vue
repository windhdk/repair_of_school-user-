    ...
    <div class="header demo-input-suffix">
      <span style="float:right;">
        <el-button-group>
          ...
          <el-button type="primary" size="mini" @click="$refs['form'].init({title: '添加', category: param.category_value})">添加轮播图</el-button>
        </el-button-group>
      </span>
    </div>
...
export default {
  ...
  methods: {
    ...
    get_data() {
      this.loading = true
      delete this.param.t
      this.$request.post('/admin/banner/getListData', this.param).then((res) => {
    ...
    del(item) {
      ...
      this.$request.post('/admin/banner/delete', data).then((res) => {
        this.$message({
          message: res.msg,
          type: 'success'
        })
